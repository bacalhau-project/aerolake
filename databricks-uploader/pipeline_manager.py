...
class PipelineManager:
    ...
    def __init__(self, db_path: str = None):
        """Initialize pipeline manager with configurable database path."""
        if db_path is None:
            # Use environment variable or default
            db_path = os.getenv("PIPELINE_CONFIG_DB", "pipeline_config.db")
    ...
    def get_current_config(self) -> Dict[str, Any]:
        ...
        if result:
            return {"type": result[0], "created_at": result[1], "source": result[2] or "database"}
        else:
    ...
    def set_pipeline_type(self, pipeline_type: str, created_by: str = "autoloader_main"):
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, 1)
        """,
            (pipeline_type, datetime.now(timezone.utc).isoformat(), created_by),
        )
...
def main():
    ...
    parser.add_argument(
        "--db",
        default=None,
        help="Database path (default: PIPELINE_CONFIG_DB env var or 'pipeline_config.db')",
    )
    ...
    if args.action == "get":
        ...
        for exec in history[:10]:
            print(
                f"  {exec['created_at']}: {exec['pipeline_type']} - {exec['records_processed']} records"
            )
