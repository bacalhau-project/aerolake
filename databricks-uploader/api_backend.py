...
async def startup_event():
    ...
    # Initialize pipeline manager with separate config database
    # IMPORTANT: Using separate database for configuration, NOT the sensor database
    config_db_path = "pipeline_config.db"
    pipeline_manager = PipelineManager(config_db_path)
...
async def get_recent_uploads(limit: int = Query(default=50, le=200)):
    ...
    try:
        ...
        return {"uploads": df.to_dict(orient="records")}
...
async def get_turbine_metrics(
    farm_id: Optional[str] = None, limit: int = Query(default=100, le=1000)
):
    ...
    try:
        ...
        df["status"] = df.apply(
            lambda row: "operational" if row["power_output"] > 0 else "idle", axis=1
        )
...
async def update_alert_config(alert_name: str, config: AlertConfig):
    ...
    return {"message": f"Alert '{alert_name}' updated successfully", "config": config}
...
async def export_report(
    ...
    try:
        ...
        if format == "csv":
            ...
            return FileResponse(
                csv_path, media_type="text/csv", filename=f"{report_type}_report.csv"
            )
...
if __name__ == "__main__":
    ...
    uvicorn.run("api_backend:app", host="0.0.0.0", port=8000, reload=True, log_level="info")
