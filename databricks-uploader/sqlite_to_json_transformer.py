...
class SensorDataTransformer:
    ...
    def transform_sqlite_with_validation(
        ...
        config = SensorReaderConfig(timeout=30.0, verbose=False)
        reader = SensorDatabaseReader(str(db_path), config)

        # Read records using the centralized reader
        records = reader.read_sensor_data(
            table_name=table_name,
            where_clause=where_clause,
            limit=limit,
            order_by=None,  # No specific ordering needed
        )

        self.logger.info(f"Processing {len(records)} records from {table_name}")
