...
class SQLiteToS3Uploader:
    ...
    def __init__(self, config_path: str, verbose: bool = False):
        ...
        # Override SQLite database path from environment variable if set
        if sqlite_path := os.getenv("SQLITE_DB_PATH"):
            self.config["sqlite"] = sqlite_path

        # Override AWS credentials directory from environment variable if set
        if creds_dir := os.getenv("AWS_CREDENTIALS_DIR"):
            if "s3_configuration" not in self.config:
    ...
    def _get_new_data(self, last_timestamp: str | None = None) -> list:
        """Get new data from SQLite since last timestamp using centralized sensor reader."""
        ...
        # Configure sensor reader with retry logic
        config = SensorReaderConfig(
            max_retries=10, initial_retry_delay=1.0, timeout=30.0, verbose=self.verbose
        )

        # Create sensor reader instance
        reader = SensorDatabaseReader(str(db_path), config)
        ...
        # Read data using centralized reader
        try:
