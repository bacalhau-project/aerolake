...
class PipelineOrchestrator:
    ...
    def __init__(self, config_path: str = "pipeline-config-v2.yaml"):
        ...
        self.stats = {"total_processed": 0, "by_scenario": {}}
    ...
    def _initialize_components(self, dry_run: bool = False):
        ...
        self.console.print(
            f"[dim]Validation spec path: {self.config['validation']['spec_path']}[/dim]"
        )
    ...
    def _read_sensor_data(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            reader = SensorDatabaseReader(str(sqlite_path), config)
            ...
            timestamp_col = self.config["sqlite"]["timestamp_col"]
            where_clause = f"{timestamp_col} > '{last_timestamp}'"

            # Read records using centralized reader
    ...
    def _process_raw_scenario(self, records: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        results = {"processed": 0, "uploaded": 0, "errors": []}
    ...
    ) -> Dict[PipelineScenario, Dict[str, Any]]:
        ...
        for scenario in active_scenarios:
            try:
                ...
                if scenario not in self.stats["by_scenario"]:
                    self.stats["by_scenario"][scenario] = {"total": 0, "successful": 0, "failed": 0}
    ...
    def _create_status_table(self) -> Table:
        ...
        for scenario in PipelineScenario:
            if scenario in active_scenarios:
                ...
                success_rate = (
                    "100%" if total == 0 else f"{(stats.get('successful', 0) / total * 100):.1f}%"
                )
                ...
                table.add_row(scenario.value, status, str(total), success_rate, details)
            else:
                table.add_row(scenario.value, "⚫ Inactive", "-", "-", "")
    ...
    def run(self, dry_run: bool = False, once: bool = False):
        ...
        with Live(self._create_status_table(), refresh_per_second=1) as live:
            while self.running:
                try:
                    # Read sensor data
                    records = self._read_sensor_data(limit=self.config["processing"]["batch_size"])
    ...
    def _complete_executions(self):
        ...
        for scenario, execution in self.current_executions.items():
            ...
            try:
                ...
                self.console.print(
                    f"[red]Failed to complete execution for {scenario.value}: {e}[/red]"
                )
